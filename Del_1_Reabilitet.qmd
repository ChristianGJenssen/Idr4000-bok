...
```{r}
#| echo: false
#| fig-cap: "***Figur 1:** Boksplot av vo2maks på testdag 1 (T1) og Testdag 2 (T2)*"

data <- read_excel("Fysologisk testing.xlsx")

ggplot(data, aes(x = factor(timepoint), y = vo2max, fill = factor(timepoint))) +
  geom_boxplot() +
  labs(title = "Vo2maks per test",
       x = "Tidspunkt",
       y = "vo2maks (ml/min/kg)") +
  theme_minimal() +
theme(plot.title = element_text(hjust = 0.51)) +
  
  ggtitle("Vo2maks Boksplot") 
```

```{r}
#| echo: false
#| fig-cap: "***Figur 2:** Figuren viser Vo2maks målinger fra T1 til T2.*"
...
```{r}
#| echo: false
#| fig-cap: "***Tabell 1:** Data er presentert som gjennomsittet, standardavviket, median og minimums- og maksimumsverdi. VO2-maks = maksimalt oksygenopptak; MAOD.a = maksimalt akkumulert oksygenunderskudd.*"
...
summary_table %>% 
  kable(caption="Karakterstikker for T1")
...
summary_table <- tibble(
  ...
  cv_vo2maks = cv_results_vo2max$cv_vo2max,
  cv_wmaks = cv_results_wmax$cv_wmax,
  cv_maoda = cv_results_maoda$cv_maoda,
  sd_dev_vo2maks = std_dev_by_id$std_dev_vo2max,
  sd_dev_wmaks = std_dev_wmax_by_id$std_dev_wmax,
  sd_dev_MAODa = std_dev_maoda_by_id$std_dev_MAODa
)
...
result_table1 <- data.frame(
  Variable = c("VO2maks", "W.maks", "MAOD.a"),
...
colnames(result_table1) <- c("Variabel", "CV", "SWC")
...
result_table1 %>% 
    kable(caption="***Tabell 3:** Gjennomsnittlig variasjonskoeffisient (%) og minste betdyningsfulle forskjell (N = 7).*")
